...
class Exp(BaseExp):
    def __init__(self):
        ...
        self.input_size = (448, 448)  # (height, width)
    ...
    def get_data_loader(
        ...
        dataset = vid.VIDDataset(file_path_rgb=self.vid_train_path,
        ...
        dataset_ir = vid.VIDDataset(file_path_rgb=self.ir_vid_train_path,
                                 img_size=self.input_size,
                                 preproc=TrainTransform(
                                     max_labels=50,
                                     flip_prob=self.flip_prob,
    ...
    def get_eval_loader(self, batch_size, tnum=None, data_num_workers=8,formal=False):
        ...
        dataset_val = vid.VIDDataset(file_path_rgb=self.vid_val_path,
        ...
        dataset_val_ir = vid.VIDDataset(file_path_rgb=self.ir_vid_val_path,
                                     img_size=self.test_size, preproc=Vid_Val_Transform(), lframe=self.lframe_val,
                                     gframe=self.gframe_val, val=True, dataset_pth=self.data_dir, tnum=tnum,formal=formal,
                                     traj_linking=self.traj_linking, local_stride=self.local_stride,)


        val_loader_rgb = vid.vid_val_loader(batch_size=batch_size,
                                        data_num_workers=data_num_workers,
                                        dataset=dataset_val, )


        val_loader_ir = vid.vid_val_loader(batch_size=batch_size,
                                        data_num_workers=data_num_workers,
                                        dataset=dataset_val_ir, )

        return val_loader_rgb, val_loader_ir

    # rewrite evaluation func
    def get_evaluator(self, val_loader, val_loader_ir):
        from yolox.evaluators.vid_evaluator_v2 import VIDEvaluator

        # val_loader = self.get_eval_loader(batch_size, is_distributed, testdev, legacy)
        evaluator = VIDEvaluator(
            dataloader=val_loader,
            dataloader_ir = val_loader_ir,
