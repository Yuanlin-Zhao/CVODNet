...
class VIDDataset(torchDataset):
    ...
    def __init__(
        ...
        self.file_path_rgb = file_path_rgb#这里是可见光npy路径
        #self.file_path_ir = file_path_ir #红外
        #print(f'file_path={file_path}')

        #print(f'可见光{file_path}, 红外{ir_file_path}')
        ...
        self.res = self.photo_to_sequence(self.file_path_rgb,
                                              lframe,gframe)
    ...
    def photo_to_sequence(self,dataset_path_rgb,  lframe,gframe):

        res_rgb = []
        #res_ir = []
        dataset_rgb = np.load(dataset_path_rgb,allow_pickle=True).tolist()
        #dataset_ir = np.load(dataset_path_ir,allow_pickle=True).tolist()
        for element in dataset_rgb:
            #print(f'element={element}')
            #element=['D:\\aa-duomotaishipinjiance\\YOLOV-master\\datasets\\Data\\000.jpg'
            ele_len = len(element)
                ...
                if self.mode == 'continous':
                    if lframe == 0:
                        ...
                        for i in range(split_num):
                            print(f'element[i * gframe:(i + 1) * gframe]={element[i * gframe:(i + 1) * gframe]}')
                            res_rgb.append(element[i * gframe:(i + 1) * gframe])

                            #res_ir.append(dataset_ir[i * gframe:(i + 1) * gframe])
                            #实际上只执行到上面
                        ...
                        if self.local_stride==1:
                            ...
                            for i in range(split_num):
                                ...
                                res_rgb.append(l_frame + g_frame)
                            ...
                            for i in range(split_num):
                                for j in range(self.local_stride):
                                    res_rgb.append(element[lframe * self.local_stride * i:lframe * self.local_stride * (i + 1)][
                                                   j::self.local_stride])
                    else:
                    ...
                    for i in range(split_num):
                        res_rgb.append(all_uniform_frame[i::split_num])
                else:
                    print('unsupport mode, exit')
                    exit(0)
        ...
        if self.val:
            if self.tnum == -1:
                ...
                return res_rgb[:self.tnum]
        else:
            #print(f'random.shuffle(res)')
            #print(f'len(res_rgb)={len(res_rgb)}, len(res_ir)={len(res_ir)}')
            #random.shuffle(res_rgb)
...
def get_vid_loader(batch_size,data_num_workers,dataset):
    #print(f'dataaaaaaaaaaaaaaaaaaaaaaaaaaa={get_vid_loader}')
