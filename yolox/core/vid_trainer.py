...
class Trainer:
    def __init__(self, exp, args, val_loader, val_loader_ir,val=False):
        ...
        self.val_loader_ir = val_loader_ir
    ...
    def train_one_iter(self):
        ...
        inps_rgb, targets_rgb,_ = self.prefetcher_rgb.next()
        inps_rgb = inps_rgb.to(self.data_type)
        targets_rgb = targets_rgb.to(self.data_type)
        targets_rgb.requires_grad = False
        inps_rgb, targets_rgb = self.exp.preprocess(inps_rgb, targets_rgb, self.input_size,
                                            )
        inps_ir, targets_ir,_ = self.prefetcher_ir.next()
        inps_ir = inps_ir.to(self.data_type)
        targets_ir = targets_ir.to(self.data_type)
        targets_ir.requires_grad = False
        inps_ir, targets_ir = self.exp.preprocess(inps_ir, targets_ir, self.input_size,
                                            )
        data_end_time = time.time()

        with torch.cuda.amp.autocast(enabled=self.amp_training):

            outputs = self.model(inps_rgb,inps_ir, targets_rgb, targets_ir,lframe = self.exp.lframe,gframe = self.exp.gframe)
    ...
    def before_train(self):
        ...
        self.train_loader_rgb = self.train_loader[0]

        self.train_loader_ir = self.train_loader[1]


        logger.info("init prefetcher, this might take one minute or less...")
        self.prefetcher_rgb = DataPrefetcher(self.train_loader_rgb)

        self.prefetcher_ir = DataPrefetcher(self.train_loader_ir)

        self.max_iter = int(self.prefetcher_rgb.max_iter)
        ...
        self.evaluator = self.exp.get_evaluator(
            ...
            val_loader_ir=self.val_loader_ir
        )
        # Tensorboard logger
        if self.rank == 0:
    ...
    def before_epoch(self):
        ...
        if self.epoch + 1 >= self.max_epoch - self.exp.no_aug_epochs or self.no_aug:
            ...
            self.train_loader_rgb.dataset.enable_mosaic = False
            self.prefetcher_rgb = vid.DataPrefetcher(self.train_loader_rgb)

            self.train_loader_ir.dataset.enable_mosaic = False
            self.prefetcher_ir = vid.DataPrefetcher(self.train_loader_ir)
            ...
            self.train_loader_rgb.dataset.enable_mosaic = False
            self.prefetcher_rgb = vid.DataPrefetcher(self.train_loader_ir)
            self.train_loader_ir.dataset.enable_mosaic = False
            self.prefetcher_ir = vid.DataPrefetcher(self.train_loader_ir)
            ...
            self.train_loader_rgb.dataset.enable_mosaic = True
            self.prefetcher_rgb = vid.DataPrefetcher(self.train_loader_rgb)
            self.train_loader_ir.dataset.enable_mosaic = True
            self.prefetcher_ir = vid.DataPrefetcher(self.train_loader_ir)
