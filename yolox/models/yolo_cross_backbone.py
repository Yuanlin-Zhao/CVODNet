...
from .yolo_cross_darknet import Cross_CSPDarknet

class YOLO_CROSS_PAFPN(nn.Module):
    """
    ...
    def __init__(
        ...
        super().__init__()
        self.backbone = Cross_CSPDarknet(depth, width, depthwise=depthwise, act=act)
    ...
    def forward(self, input, ir_input):
        #  backbone
        x2, x1, x0 = self.backbone(input, ir_input)
...
class YOLOPAFPN_Swin(nn.Module):
    ...
    def __init__(
        ...
        self.backbone = SwinTransformer(out_indices=in_features, depths=swin_depth, num_heads=num_heads,
                                        embed_dim=base_dim, pretrain_img_size=pretrain_img_size, ape=ape,
